...
function FeedPageContent() {
  ...
  useEffect(() => {
    ...
    const loadStories = async () => {
      ...
      const [{ data: profilesData }, metricsByStory] =
        await Promise.all([
          authorIds.length > 0
  ...
  const updateQueryParams = (next: { q?: string; mode?: FeedMode; view?: FeedView }) => {
    ...
    router.push(finalQuery ? `/feed?${finalQuery}` : "/feed");
  };
  ...
  const sendAction = async (
    action: "save" | "dismiss" | "snooze" | "open",
    story: FeedStory,
  ) => {
  ...
  const applySwipeAction = async (
    action: "save" | "dismiss" | "snooze",
    story: FeedStory,
  ) => {
  ...
  const handleCoverTouchEnd = async (
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {(["following", "feed", "public-domain"] as const).map((modeOption) => (
            <button
              key={modeOption}
              type="button"
